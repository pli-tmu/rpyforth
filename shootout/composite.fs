\ Composite benchmark: runs the six micro-benchmarks in one process, in fixed
\ order (ack, fibo, sieve, nestedloop, ary, heap), and prints each result plus a
\ combined checksum. A mixed multi-workload run, so a single benchmark cannot be
\ tuned in isolation.

8     constant ACK-N
40    constant FIB-N
8192  constant SIEVE-SIZE
10000 constant SIEVE-ITERS
30    constant NL-N
8000  constant ARY-N
1000  constant ARY-REPS
4000  constant HEAP-N
...
: composite ( -- sum )
    0
    ACK-N 3 ack   ." ack: "        dup 1 u.r cr  +
    FIB-N fib     ." fib: "        dup 1 u.r cr  +
    sieve-bench   ." sieve: "      dup 1 u.r cr  +
    0 nestedloops ." nestedloop: " dup 1 u.r cr  +
    ary Y @ @     ." ary: "        dup 1 u.r cr  +
    heap-run      ." heap: "       dup 1 u.r cr  + ;

UTIME 2>R
composite ." checksum: " 1 u.r cr
