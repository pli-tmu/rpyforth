...
nothing. (This replaced an earlier design in which each call allocated a
fragment node linked to its parent — see *History* below.)

## Where the management lives

Three layers, from policy to mechanics:

1. **When to push/pop — `rpyforth/inner_interp.py`, at call boundaries.**
   ...
   return. All gated by `USE_STACK_FRAGMENT`.
2. **Dispatch wrappers — `rpyforth/metastack.py`.** `push_ds_fragments` /
   `pop_ds_fragments_commit` / `reset_ds_fragments` forward to the active
   metastack's methods and also drive the float fragment when it is enabled.
3. **The mechanics — `rpyforth/metastack_int.py`.** `push_fragment_on`,
...
rather than a memory load — in JIT logs it appears as `inst_cache_depth`, and the
...
(The field was historically named `d`; it was renamed so those guards are legible
in traces.)
...
slide the frame down), then proceeds. Underflow past `cache_depth == 0` reads
straight from the spill top. Neither happens for stacks that stay within 10 cells.
...
`cache_depth > NTOP` — copies the below-NTOP frame cells down into the spill,
...
Return needs no copy-back. The spill already holds the caller's cells
contiguously below the callee's result, so `pop_fragment_commit` is **O(1)** — it
just decrements `frag_ptr`. This is the property that removed the per-call
...
CATCH; it does not index the spill (the spill window is defined by `spill_ptr`).

## CATCH and snapshot / restore

CATCH saves and restores the whole cache alongside the call stack. On
`catch_push_frame` (`inner_interp.py`) the scalar tops, `cache_depth`, `frag_ptr`,
`spill_ptr`, and the frame row are copied into per-frame arrays
(`ca_t0`, `ca_t1`, `ca_cache_depth`, `ca_frag`, `ca_spill`, `ca_frames`); the
spill *contents* below `spill_ptr` are left in place and are still valid because
they were never overwritten. `throw_unwind` restores those fields, which rolls
the logical stack back to the CATCH point regardless of how deep the protected
word grew.

`snapshot_cache` / `restore_cache` (`metastack_int.py`) provide the same
...
parent, forming a linked list down the call chain. That cost a heap allocation
per call (GC traffic) and a pointer chase per deep access, and — decisively for
the meta-tracing JIT — a linked list of heap nodes cannot be virtualized.
...
the range `[0, spill_ptr)`, and nest/unnest only moves `spill_ptr` and
`frag_ptr`. Nothing is allocated per call, and the live cache is a small fixed
set of virtualizable fields the JIT can hold in registers across a trace.
...
priority order (frame-only > parametric-NTOP > default), plus one optional addon:
...
- **frame-only** (`RPYFORTH_FRAME_ONLY=1`, conceptually NTOP=0,
  ...
  virtualizable `frame[*]` array, so push/pop move no data (just an index),
  mirroring PyPy's own `fastlocals_w[*]`.
- **parametric-NTOP** (`RPYFORTH_NTOP` set to 2/4/8/16,
  `metastack_int_ntop.py`) — generalizes the scalar-tops count to
  `EFFECTIVE_NTOP`. The call window stays `CALL_WINDOW = NTOP = 2` for
  calling-convention parity across every NTOP value.
- **float fragment** (`RPYFORTH_FLOAT_FRAGMENT=1`) — an addon that mirrors this
  layout for the float stack. It rides only on the default int layout and was
  measured a net loss, so it is opt-in for ablation.
...
`STACK_FRAGMENT_VIRTUALIZABLES`: the int cache fields
(`t0`, `t1`, `cache_depth`, `frame[*]`, `frag_ptr`, `spill_ptr`), the return- and
call-stack pointers, and the loop / cell-size state. The `spill` array itself is
**not** in this set: it is one immutable-reference array, too large to
virtualize, touched only when the stack goes deeper than the ~10-cell cache.
...
The net effect is that the shallow, hot part of the stack is pure register
arithmetic inside a trace, deep and recursive stacks cost no per-call allocation,
and only the rare deep access falls through to plain-heap memory.
