\ global-5-sum-elite-init.fs
\ 	$Id: global-5-sum-elite-init.fs,v 1.6 2005/04/10 16:20:27 f Exp $	

\ Initialisation file for global-5-sum.fs.

\ Brew must add five global integer variables and put the sum into organ-A
\ A = B + C + D + E + F
\ The task is surprisingly difficult for brew...

false [IF] \ some results:

GENE: g-10999
    integer-E@ integer-C@ + integer-D@ dup negate integer-B@ integer-F@
    integer-D@ + + + + + organ-A !
;gene
    
\ It needs a looong time, (especially to get rid of useless code).
GENE: g-53028
    integer-E@ integer-C@ + integer-F@ integer-D@ integer-B@ + + + organ-A !
;gene
...
0 |B OR |C OR |D OR |E OR |F OR  global-diversification-mask !
...
9000 ' snip-types	mutation-types set-as-sublist
...
1000 	internal+' integer-E@	actual-genepool-xt @ execute set-one
1000 	internal+' integer-F@	actual-genepool-xt @ execute set-one
...
INCLUDE INPUTS/experiments/sum/global-5-sum.fs 
...
s" Initialisation file 'global-5-sum-elite-init.fs' included.  Experiment ready."
1 >message

menu-leave on		\ go to top level menu = brew main screen
single-step on		\ might be off from before

CR .( ELITISM MODE ) 300 MS
elitism!
120 fixed-population-size !
40 elite !
1 1 mutation-rate 2!
