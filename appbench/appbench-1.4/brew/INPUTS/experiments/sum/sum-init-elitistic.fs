\ sum-init-elitistic.fs
\ 	$Id: sum-init-elitistic.fs,v 1.6 2005/04/10 16:20:28 f Exp $	

\ This is an initialisation file corresponding to 'sum.fs'.
\ Elitistic mode:  gives very quick results here.
...
GENE: g-148
    C-property@ parameter-B@ +
    parameter-B@ organ-B !(some)
    organ-A !
;gene
...
\ 'brew' recorded evolutionary session playback file. Hand edited a bit.
\
...
1000 diversification-range !
1 1 diversification-rate 2!
...
0 organ-G !
0 parameter-A !
0 parameter-B !		\ invitation to cheat ;-)
0 parameter-C !
0 parameter-D !
0 parameter-E !
0 invisible-A !
...
s" Initialisation file 'sum-init-elitistic.fs' included.  Experiment ready."  1 >message
