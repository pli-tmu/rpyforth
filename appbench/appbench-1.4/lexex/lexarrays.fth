\ LexArrays.fth converts the transition table into the data structures to
\ be used by the lexical scanner (See Aho, Sethi & Ullman, p145, Fig 3.47

\ Version 1.1 28 December 2006 - Improved ambiguity resolution
...
\ but we need to compile references to it hence the two part definition
...
: saveLexToken ( 2un...2u1 n state )
   ...
   if          ( 2u1 1 state ) \ 2u1 = tok pos
      nip nip                    ( -- tok state )
      lex_tokens !               ( -- )
   else        \ n > 1, only use a unique acceptor
      0 rot 0                    ( -- ... 2u1 state ct n 0 )
      ?do
         2>r                     ( -- ... 2u1 ) ( R: -- state ct )
         uniqueAcceptor?         ( -- ... 2u2 tok1 f )
         if
            dup r> 1+            ( -- ... 2u2 tok1 tok1 ct' )
            r@ swap >r           ( -- ... 2u2 tok1 tok1 state )
            lex_tokens !         ( -- ... 2u2 tok1 )
         then
         drop 2r>                ( -- ... 2u2 state ct )
      loop
      1 > abort" saveLexToken: can't have more than 1 unique acceptor"
      drop                       ( -- )
   then
;

[else]   \ New version 21/12/06
