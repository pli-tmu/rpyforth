...
\     then bits/cell. This would allow more efficient operations on such sets.
...
object class
   ...
   1 cells class-var maxMember   \ maxMember does not appear to be used anywhere ??
                                 \ But it could be used in get-next-member
end-class SetBase
...
\ Suggested improvement is to use a magic number

: get-next-member   ( set u -- n )
   over @                        ( -- set u class )
   cells/set @ bits/cell * swap
   ?do                           ( -- set )
      dup i member?
      if
         drop i unloop exit
